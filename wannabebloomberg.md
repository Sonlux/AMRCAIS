...
| **Correlation anomaly vs regime baseline** | Shows current correlations flat | Flags when correlations deviate from what's *expected for the current regime* — not just from zero |
...
The two are complementary, not competing. AMRCAIS could sit *on top of* Bloomberg data as a regime intelligence overlay — taking Bloomberg's data depth and adding the interpretation layer that Bloomberg lacks entirely.
